...
def test_append_session_log_creates_daily_note(tmp_path: Path):
    ...
    assert f'## Deep Work Assistant session {expected_time}' in text
    assert '- primary app: `code.exe`' in text
    assert '- reminder plan: hydration=64m, stretch=128m, eat=192m' in text
...
def test_append_session_log_appends_to_existing_note(tmp_path: Path):
    ...
    assert text.count('## Deep Work Assistant session') == 1
