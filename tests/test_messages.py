...
class TestTimeOfDay:
    def test_morning(self):
        dt = datetime(2026, 6, 2, 9, 0, tzinfo=timezone.utc)
        assert time_of_day_label(dt) == 'morning'

    def test_afternoon(self):
        dt = datetime(2026, 6, 2, 14, 0, tzinfo=timezone.utc)
        assert time_of_day_label(dt) == 'afternoon'

    def test_evening(self):
        dt = datetime(2026, 6, 2, 19, 0, tzinfo=timezone.utc)
        assert time_of_day_label(dt) == 'evening'

    def test_night(self):
        dt = datetime(2026, 6, 2, 2, 0, tzinfo=timezone.utc)
...
class TestBuildReminderMessage:
    def test_hydration_message_contains_water_keyword(self):
        ...
        assert any(word in msg.lower() for word in ('water', 'hydrate', 'sip', 'drink', 'glass', 'zone'))
    ...
    def test_streak_celebration_included(self):
        now = datetime(2026, 6, 2, 14, 0, tzinfo=timezone.utc)
        # Streak celebration is probabilistic; over repeated calls the branch
        # must appear, and when it does it must include a streak-related word.
        texts = [build_reminder_message('hydration', streak_days=5, now=now) for _ in range(20)]
        assert any('streak' in msg.lower() or 'day' in msg.lower() for msg in texts)
